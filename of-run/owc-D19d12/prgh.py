#### INPUT DATA
path='/home/lewi/Documentos/OWCESOF/cfd/wkp/CASECANAL/CANALspt/CANAL3D/14-owcD11/' #OpenFOAM folder
output_file_name="rghPressure.csv"
#g_direction="Y"
...
for t in time_steps:
    ...
    p = servermanager.Fetch(calculator1)
    p.GetPointData().GetNumberOfArrays()
    p0 = p.GetPointData().GetArray('p_rgh')
    if p is not None:
        p_val = p0.GetTuple1(0)	
        f.write("{0},{1}\n".format(t,p_val))
...
rghPressurecsv = CSVReader(FileName=[path+output_file_name])
